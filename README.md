...
<ul>
  ...
  <li><strong>Automatic Video Normalization:</strong> Local library videos that are not already <code>1920×1080</code> <code>.mp4</code> files are automatically converted into wallpaper-ready <code>1080p mp4</code> versions while preserving the source FPS, and the original file is removed after a successful conversion.</li>
</ul>
...
<ul>
  ...
  <li><strong>True Fill / Cover Playback:</strong> Single videos, playlists, and reverse playback now keep filling the monitor cleanly during aspect-ratio mismatches and screen rotation changes, with fit controls for <code>Cover</code>, <code>Contain</code>, and <code>Stretch</code>.</li>
...
<ul>
  ...
  <li><strong>Wallpaper-Ready Library Conversion:</strong> The app can automatically standardize mixed-resolution and mixed-container local videos into a consistent <code>1080p mp4</code> wallpaper library, helping playback stay predictable while keeping the original frame rate.</li>
  <li><strong>Live Video Adjustments:</strong> Brightness, contrast, saturation, gamma, hue shift, and global color tint controls for red, green, blue, yellow, cyan, and magenta can be applied directly to the wallpaper output.</li>
  <li><strong>Higher-Quality Reverse Cache:</strong> Reverse playback keeps the original frame cadence and uses a higher-quality encode path instead of aggressively downscaling the cached reverse video.</li>
...
<p>Wall Blazer includes a robust, automated installer that handles system dependencies, Python packages, and the native build process using Meson and Ninja. The Linux installer now has best-effort support for <code>apt</code>, <code>dnf</code>, <code>pacman</code>, and <code>zypper</code> based systems.</p>
...
<p>Wall Blazer now supports native Windows with an automated <strong>MSYS2/UCRT64 + GTK</strong> build path and bundled GTK/VLC runtime files.</p>
<ol>
  <li>Download the repository as a ZIP or clone it via Git.</li>
  <li>Double-click the <code>install_windows.bat</code> file.</li>
  <li>It will automatically install <code>MSYS2 UCRT64</code>, the GTK/PyGObject toolchain, <code>VLC (64-bit)</code>, build <code>Wall-Blazer.exe</code>, and place it on your Desktop and Startup folder.</li>
</ol>
